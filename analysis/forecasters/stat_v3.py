"""Stage 3.5 forecaster: StatV2 features + payday-cycle features.

Extends StatV2 with two more features derived purely from the timestamp:
  6. days_since_last_payday   : 0..14ish, monotonic since most recent payday
  7. days_until_next_payday   : 0..14ish, monotonic to next payday

Paydays in Venezuela are the 15th and the last day of each month. FX demand
spikes predictably approaching payday as workers convert salary to dollars to
preserve purchasing power. The kernel-NN should find historical days at the
same cycle-phase as today and weight their outcomes accordingly.

History rows must already be enriched (see analysis.forecasters.enrich).
Payday features are computed in-process from the row timestamp — no DB
lookups, no new tables needed.

Same kernel-NN machinery as StatV2. 7 features instead of 5. Falls back to
uniform when features are undefined or training set is too small.
"""
...
from .payday import payday_features
...
def _features_at(readings, idx):
    """Compute the 7-feature vector at readings[idx]. Returns tuple or None."""
    t, row = readings[idx]
    ...
    # ---- Stage-3 feature 5: news count ----
    news_count = row.get("news_count_7d", 0)
    if news_count is None:
        news_count = 0

    # ---- Stage-3.5 features 6 & 7: payday cycle ----
    days_since, days_until = payday_features(t)
...
class StatV3Forecaster:
    name = "stat_v3"

    def __init__(
        self,
        lookback_days: int = 30,
        kernel_sigma: float = 1.4,
        smoothing: float = 1.0,
        min_examples: int = 10,
    ):
        # kernel_sigma 1.4 (vs StatV2's 1.2): 7 features = even larger NN
        # distances on average, so the kernel is widened again.
        # min_examples 10 (vs StatV2's 8): more dims need more support.
